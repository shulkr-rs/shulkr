...
impl<Node: Eq + Hash + Copy> GraphState<Node> {
    ...
    pub fn new(level_count: i32) -> Self {
        assert!(level_count < 254, "level count must be < 254 (matches vanilla's byte-packed computed-level storage headroom)");
        Self {
            level_count,
...
pub trait DynamicGraph {
    ...
    fn calculate_priority(&self, level: i32, computed_level: i32) -> i32 {
        level.min(computed_level).min(self.state_ref().level_count - 1)
    }
    ...
    fn check_edge(&mut self, from: Self::Node, to: Self::Node, new_level_from: i32, only_decreased: bool) {
        let level_to = self.get_level(to);
        let old_computed_level = self.state_ref().computed_levels.get(&to).copied();
        self.check_edge_inner(from, to, new_level_from, level_to, old_computed_level, only_decreased);
    ...
    fn check_edge_inner(
        ...
        let new_computed_level = if only_decreased {
            ...
            self.get_computed_level(to, from, new_level_from).clamp(0, level_count - 1)
        };
        ...
        if level_to != new_computed_level {
            ...
            if old_priority != new_priority && !was_consistent {
                self.state().priority_queue.dequeue(to, old_priority as usize, new_priority as usize);
            }
            let state = self.state();
            ...
            state.priority_queue.dequeue(to, old_priority as usize, level_count as usize);
            state.computed_levels.remove(&to);
        }
    }

    fn check_neighbor(&mut self, from: Self::Node, to: Self::Node, level: i32, only_decreased: bool) {
        let level_count = self.state_ref().level_count;
        let stored_old_computed = self.state_ref().computed_levels.get(&to).copied();
        let level_from = self.compute_level_from_neighbor(from, to, level).clamp(0, level_count - 1);

        if only_decreased {
            let level_to = self.get_level(to);
            self.check_edge_inner(from, to, level_from, level_to, stored_old_computed, only_decreased);
        } else {
            ...
            if level_from == old_computed_level {
                let level_to = if was_consistent { old_computed_level } else { self.get_level(to) };
                self.check_edge_inner(from, to, level_count - 1, level_to, stored_old_computed, only_decreased);
            }
        }
    }
    ...
    fn remove_from_queue(&mut self, node: Self::Node) {
        ...
        if let Some(computed_level) = computed_level {
            ...
            let state = self.state();
            let level_count = state.level_count;
            state.priority_queue.dequeue(node, priority as usize, level_count as usize);
            state.has_work = !state.priority_queue.is_empty();
        }
    }
}
