use crate::{
    protocol::{
        ...
        packet::{
            ...
            StatusResponsePacket, SwingAnimationPacket, SyncPlayerPositionPacket,
            SystemChatMessagePacket, UnloadChunkPacket, UpdateAdvancementsPacket,
            UpdateObjectivesPacket, UpdateScorePacket, UpdateTagsPacket, WorldEventPacket,
            server::{
...
fn play(type_id: TypeId) -> Option<i32> {
    Some(match () {
        ...
        // _ if type_id == TypeId::of::<SetContainerDataPacket>() => 0x13,
        ...
        // _ if type_id == TypeId::of::<PostEffectsPacket>() => 0x52,
        _ if type_id == TypeId::of::<RespawnPacket>() => 0x53,
        _ if type_id == TypeId::of::<SetHeadRotationPacket>() => 0x54,
        // _ if type_id == TypeId::of::<SectionBlocksUpdatePacket>() => 0x55,
        // _ if type_id == TypeId::of::<SelectAdvancementsTabPacket>() => 0x56,
        // _ if type_id == TypeId::of::<ServerDataPacket>() => 0x57,
        // _ if type_id == TypeId::of::<SetActionBarTextPacket>() => 0x58,
        // _ if type_id == TypeId::of::<SetBorderCenterPacket>() => 0x59,
        // _ if type_id == TypeId::of::<SetBorderLerpSizePacket>() => 0x5A,
        // _ if type_id == TypeId::of::<SetBorderSizePacket>() => 0x5B,
        // _ if type_id == TypeId::of::<SetBorderWarningDelayPacket>() => 0x5C,
        // _ if type_id == TypeId::of::<SetBorderWarningDistancePacket>() => 0x5D,
        // _ if type_id == TypeId::of::<SetCameraPacket>() => 0x5E,
        _ if type_id == TypeId::of::<SetCenterChunkPacket>() => 0x5F,
        // _ if type_id == TypeId::of::<SetChunkCacheRadiusPacket>() => 0x60,
        _ if type_id == TypeId::of::<SetCursorItemPacket>() => 0x61,
        // _ if type_id == TypeId::of::<SetDefaultSpawnPositionPacket>() => 0x62,
        _ if type_id == TypeId::of::<DisplayObjectivePacket>() => 0x63,
        _ if type_id == TypeId::of::<SetEntityMetadataPacket>() => 0x64,
        // _ if type_id == TypeId::of::<SetEntityLinkPacket>() => 0x65,
        // _ if type_id == TypeId::of::<SetEntityMotionPacket>() => 0x66,
        // _ if type_id == TypeId::of::<SetEquipmentPacket>() => 0x67,
        // _ if type_id == TypeId::of::<SetExperiencePacket>() => 0x68,
        // _ if type_id == TypeId::of::<SetHealthPacket>() => 0x69,
        _ if type_id == TypeId::of::<SetHeldItemPacket>() => 0x6A,
        _ if type_id == TypeId::of::<UpdateObjectivesPacket>() => 0x6B,
        // _ if type_id == TypeId::of::<SetPassengersPacket>() => 0x6C,
        // _ if type_id == TypeId::of::<SetPlayerInventoryPacket>() => 0x6D,
        // _ if type_id == TypeId::of::<SetPlayerTeamPacket>() => 0x6E,
        _ if type_id == TypeId::of::<UpdateScorePacket>() => 0x6F,
        // _ if type_id == TypeId::of::<SetSimulationDistancePacket>() => 0x70,
        // _ if type_id == TypeId::of::<SetSubtitleTextPacket>() => 0x71,
        // _ if type_id == TypeId::of::<SetTimePacket>() => 0x72,
        // _ if type_id == TypeId::of::<SetTitleTextPacket>() => 0x73,
        // _ if type_id == TypeId::of::<SetTitlesAnimationPacket>() => 0x74,
        // _ if type_id == TypeId::of::<SoundEntityPacket>() => 0x75,
        // _ if type_id == TypeId::of::<SoundPacket>() => 0x76,
        // _ if type_id == TypeId::of::<StartConfigurationPacket>() => 0x77,
        // _ if type_id == TypeId::of::<StopSoundPacket>() => 0x78,
        // _ if type_id == TypeId::of::<StoreCookiePacket>() => 0x79,
        _ if type_id == TypeId::of::<SwingAnimationPacket>() => 0x7A,
        _ if type_id == TypeId::of::<SystemChatMessagePacket>() => 0x7B,
        _ if type_id == TypeId::of::<SetTablistHeaderFooterPacket>() => 0x7C,
        // _ if type_id == TypeId::of::<TagQueryPacket>() => 0x7D,
        // _ if type_id == TypeId::of::<TakeItemEntityPacket>() => 0x7E,
        // _ if type_id == TypeId::of::<TeleportEntityPacket>() => 0x7F,
        // _ if type_id == TypeId::of::<TestInstanceBlockStatusPacket>() => 0x80,
        // _ if type_id == TypeId::of::<TickingStatePacket>() => 0x81,
        // _ if type_id == TypeId::of::<TickingStepPacket>() => 0x82,
        // _ if type_id == TypeId::of::<TransferPacket>() => 0x83,
        _ if type_id == TypeId::of::<UpdateAdvancementsPacket>() => 0x84,
        // _ if type_id == TypeId::of::<UpdateAttributesPacket>() => 0x85,
        // _ if type_id == TypeId::of::<UpdateMobEffectPacket>() => 0x86,
        // _ if type_id == TypeId::of::<UpdateRecipesPacket>() => 0x87,
        // _ if type_id == TypeId::of::<UpdateTagsPacket>() => 0x88,
        // _ if type_id == TypeId::of::<ProjectilePowerPacket>() => 0x89,
        // _ if type_id == TypeId::of::<CustomReportDetailsPacket>() => 0x8A,
        // _ if type_id == TypeId::of::<ServerLinksPacket>() => 0x8B,
        // _ if type_id == TypeId::of::<WaypointPacket>() => 0x8C,
        // _ if type_id == TypeId::of::<ClearDialogPacket>() => 0x8D,
        // _ if type_id == TypeId::of::<ShowDialogPacket>() => 0x8E,
        _ => return None,
    })
}
